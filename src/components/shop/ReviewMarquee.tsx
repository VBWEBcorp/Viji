"use client";

interface Review {
  name: string;
  initial: string;
  when: string;
  body: string;
}

interface Props {
  reviews: Review[];
}

export default function ReviewMarquee({ reviews }: Props) {
  // Repeat the list so the CSS marquee loop is seamless on wide viewports.
  // The animation goes 0 → -50%, so we need an even number of copies (the second half
  // must be identical to the first). 4 copies = first half is 2 sets, plenty of width.
  const loop = [...reviews, ...reviews, ...reviews, ...reviews];

  return (
    <div className="relative overflow-hidden">
      {/* Soft fade on edges */}
      <div className="pointer-events-none absolute inset-y-0 left-0 w-16 md:w-24 bg-gradient-to-r from-white to-transparent z-10" />
      <div className="pointer-events-none absolute inset-y-0 right-0 w-16 md:w-24 bg-gradient-to-l from-white to-transparent z-10" />

      <div
        className="flex gap-6 md:gap-8 animate-marquee-reverse w-max"
        style={{ animationDuration: "60s" }}
      >
        {loop.map((r, i) => (
          <article
            key={`${r.name}-${i}`}
            className="w-[300px] md:w-[360px] shrink-0 border-t border-[var(--brand-gold)]/30 pt-6"
          >
            <div className="flex items-center gap-3 mb-4">
              <div className="w-9 h-9 rounded-full bg-[var(--brand-cream)] text-[var(--brand-gold)] flex items-center justify-center text-sm font-medium">
                {r.initial}
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-[13px] font-medium text-gray-900 leading-tight truncate">
                  {r.name}
                </p>
                <p className="text-[11px] text-gray-400">{r.when}</p>
              </div>
              <GoogleLogo size={14} className="opacity-50 shrink-0" />
            </div>
            <p className="text-[13px] text-gray-700 leading-relaxed italic">
              «&nbsp;{r.body}&nbsp;»
            </p>
          </article>
        ))}
      </div>
    </div>
  );
}

function GoogleLogo({ size = 16, className = "" }: { size?: number; className?: string }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" className={className} aria-hidden="true">
      <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" />
      <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" />
      <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" />
      <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" />
    </svg>
  );
}
